...
| 区域       | 功能说明                                                                 |
|------------|--------------------------------------------------------------------------|
| 顶部工具栏 | 包含选择文件夹、批量添加、全选/取消全选、切换模式等操作按钮               |
| 内容区域   | 显示图片缩略图、文件名、单词输入框及创建卡片按钮，支持滚动查看多文件       |
| 模式标签   | 显示当前语言模式（日语/英语）                                             |

## 五、使用流程

### 步骤 1：配置 API 密钥
在 `main.py` 中修改以下配置（或通过环境变量设置）：
```python
self.openai_config = {
...
### 步骤 2：启动 AnkiConnect 服务
1. 打开 Anki
2. 进入「工具」→「AnkiConnect」→ 确保「启动时自动启动服务器」已勾选
3. 确认服务运行在 `http://localhost:8765`（默认端口）

### 步骤 3：处理图片文件
1. 点击「选择文件夹」按钮，选择包含目标图片的文件夹（支持 .png, .jpg, .jpeg, .gif, .bmp 格式）
2. 内容区域将显示所有图片的缩略图、文件名及输入框
3. 在「单词」输入框中填写需要解析的目标词汇（需存在于图片内容中）

### 步骤 4：创建 Anki 卡片
- **单张创建**：点击对应图片的「创建卡片」按钮，系统将自动：
  1. 调用 OpenAI 解析单词信息（原型、发音、释义等）
  2. 压缩图片并上传至 Anki 媒体库
  3. 生成包含图片、发音、例句的 Anki 卡片
- **批量创建**：
  1. 通过「全选」或手动勾选需要处理的图片
  2. 点击「批量添加」按钮，系统将批量执行单张创建流程

### 步骤 5：切换语言模式
点击「切换模式」按钮可在日语/英语模式间切换，模式标签会同步更新状态。切换后：
- 日语模式：使用日语词典解析规则，卡片存入「日本語::ランダム::アニメ・マンガ・マスコミ」牌组
- 英语模式：使用英语词典解析规则，卡片存入「English Vocabulary::A English Daily」牌组
